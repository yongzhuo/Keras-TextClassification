...
# @time     :2020/4/1 19:51
# @author   :Mo
# @function :train of LEAM with baidu-qa-2019 in question title or sim-webank
...
from keras_textclassification.conf.path_config import path_baidu_qa_2019_train, path_baidu_qa_2019_valid, \
                                                      path_sim_webank_train, path_sim_webank_valid
# 数据预处理, 删除文件目录下文件
from keras_textclassification.data_preprocess.text_preprocess import PreprocessText, PreprocessSim, delete_file
# 模型图
from keras_textclassification.m16_LEAM.graph import LEAMGraph as Graph
# 计算时间
import time
...
def train_sim(hyper_parameters=None, rate=1.0):
    """
        训练函数
    ...
    if not hyper_parameters:
        hyper_parameters = {
            ...
            'model': {'label': 2,  # 类别数
                      ...
                      'loss': "categorical_crossentropy", # 'binary_crossentropy', # 损失函数
                      'metrics': 'accuracy',  # 保存更好模型的评价标准
                      'optimizer_name': 'Adam',  # 优化器, 可选['Adam', 'Radam', 'RAdam,Lookahead'], win10下必须使用GPU, 原因未知
                      'is_training': True,  # 训练后者是测试模型
                      'model_path': path_model,
